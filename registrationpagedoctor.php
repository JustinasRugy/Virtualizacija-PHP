    ...
    <style>
        * { -webkit-box-sizing:border-box; -moz-box-sizing:border-box; -ms-box-sizing:border-box; -o-box-sizing:border-box; box-sizing:border-box; }
        ...
        input {
            ...
            border-radius: 4px;
            box-shadow: inset 0 -5px 45px rgb(219, 167, 255), 0 1px 1px rgba(255,255,255,0.2);
...
if (isset($_REQUEST['username'])) {
    ...
    $Vardas = stripslashes($_REQUEST['Vardas']);
    $Vardas = mysqli_real_escape_string($con, $Vardas);
    $Pavarde = stripslashes($_REQUEST['Pavarde']);
    $Pavarde = mysqli_real_escape_string($con, $Pavarde);
    $password = stripslashes($_REQUEST['password']);
    $password = mysqli_real_escape_string($con, $password);
    $GydytojoTipas = stripslashes($_REQUEST['GydytojoTipas']);
    $GydytojoTipas = mysqli_real_escape_string($con, $GydytojoTipas);
    $Pacientu_kiekis = stripslashes($_REQUEST['PacientuKiekis']);
    $Pacientu_kiekis = mysqli_real_escape_string($con, $Pacientu_kiekis);
    $trn_date = date("Y-m-d H:i:s");
    $query = "INSERT into `users` (username, password, Vardas, trn_date, Pavarde, GydytojoTipas, Pacientu_kiekis, statusas,gydytojas_pacientas)
                        VALUES ('$username', '" . md5($password) . "', '$Vardas', '$trn_date', '$Pavarde', '$GydytojoTipas', '$Pacientu_kiekis',TRUE, TRUE)";
...
<div style="text-align: center;" class ="login">
    ...
    <form name="registration" action="" method="post" class="px-4 py-3">
        ...
        <div class="form-group">
            <input type="text" name="GydytojoTipas" placeholder="Gydytojo tipas" required /><br>
        </div>
        ...
        <div class="form-group">
            <input type="submit" name="submit" value="Registruotis" class="button-two"/>
        </div>
    </form>
    <div class='form'>
